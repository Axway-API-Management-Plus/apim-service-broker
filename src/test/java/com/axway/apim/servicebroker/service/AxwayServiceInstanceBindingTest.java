...
public class AxwayServiceInstanceBindingTest extends BaseClass {
	...
	public void shouldCreateServiceBinding() throws Exception {
		...
		BindResource bindResource = BindResource.builder().properties(bind_resource).route("testapp.axway.com").build();

		CreateServiceInstanceBindingRequest createServiceInstanceBindingRequest = CreateServiceInstanceBindingRequest
				.builder().bindResource(bindResource).planId(plan_id).serviceInstanceId(service_id).parameters(parameters).build();
	...
	public void shouldDeleteServiceBinding() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders
				...
				.header("X-Broker-API-Originating-Identity",
						"cloudfoundry eyANCiAgInVzZXJfaWQiOiAiNjgzZWE3NDgtMzA5Mi00ZmY0LWI2NTYtMzljYWNjNGQ1MzYwIiwNCiAgInVzZXJfbmFtZSI6ICJqb2VAZXhhbXBsZS5jb20iDQp9")
